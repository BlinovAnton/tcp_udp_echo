...
int tcp_listen_fd = 0, tcp_accept_fd = 0, udp_socket = 0, epollfd = 0;
char *semaph_name = "semka_file";

void sigint_hand (int);

int main () {
    int i = 0, port = 8080, bytes = 0, status = 0, nfds = 0;
    int  bind_res = 0, cli_struct_len = 0;
    ...
    *end_pid_cer = 0;
    ...
    if (epollfd == -1) {
	printf("epoll_create() fault\n");
	perror(":::");
	goto error_in_parent;
    }
    ev[0].events = EPOLLIN;
    ev[0].data.fd = tcp_listen_fd;
    if (epoll_ctl(epollfd, EPOLL_CTL_ADD, tcp_listen_fd, &ev[0]) == -1) {
	printf("epoll_ctl(tcp_listen_fd) fault\n");
	perror(":::");
	goto error_in_parent;
    }
    ev[1].events = EPOLLIN;
    ev[1].data.fd = udp_socket;
    if (epoll_ctl(epollfd, EPOLL_CTL_ADD, udp_socket, &ev[1]) == -1) {
	printf("epoll_ctl(tcp_listen_fd) fault\n");
	perror(":::");
	goto error_in_parent;
    }
	...
	for (i = 0; i < nfds; i++) {
		...
		pid = fork();
		//+1 to counter of pid, which not run exit(0) yet
		start_pid_cer++;
		switch (pid) {
		    case 0:
			...
			if ((bytes = recv(tcp_accept_fd, buf_for_recv, COND_MSG_SIZE, 0)) <= 0) {
			    ...
			    printf("tspSERVER(%s:%d) recv (%d bytes) from client(%s:%d): %s\n",
					server_ip, tcp_port, bytes, client_ip,
					ntohs(client_info.sin_port), buf_for_recv);
			}
			sleep(5);
			...
			printf("\n");
			close(tcp_accept_fd);
			//+1 to counter of pid, which already run exit(0)
...
error_in_child:
    ...
    if (close(epollfd) == -1) printf("[%d] close(epollfd) fault\n", getpid());
    exit(1);
...
void sigint_hand (int signal) {
	...
	/* exit_code = 0; //good idea but smth gone wrong
	goto end_parrent */
	if (close(tcp_listen_fd) == -1) printf("(%d) close(listen) for nothing\n", main_pid);
	if (close(tcp_accept_fd) == -1) printf("(%d) close(accept) for nothing\n", main_pid);
	if (close(udp_socket)    == -1) printf("(%d) close(udp_sock) for nothing\n", main_pid);
